...
// Client handles HTTP communication with the CPM ipv4-res API V1.
// Docs: https://cz.resvn.net/billing-docs-res-v1
...
// NewClient creates a CPM API client.
...
func (c *Client) do(ctx context.Context, method, path string, body any, out any) error {
	...
	if body != nil {
		...
		if err != nil {
			return fmt.Errorf("cpm: marshal request: %w", err)
		}
		bodyBytes = b
	...
	if err != nil {
		return fmt.Errorf("cpm: build request: %w", err)
	}
	...
	for attempt := 0; attempt < maxRetries; attempt++ {
		...
		if err != nil {
			lastErr = fmt.Errorf("cpm: http do: %w", err)
			continue
		}
		...
		if out != nil && len(respBytes) > 0 {
			// CPM wraps responses in { success, data, error }
			var apiResp apiResponse
			if err := json.Unmarshal(respBytes, &apiResp); err != nil {
				return fmt.Errorf("cpm: unmarshal envelope: %w", err)
			}
			...
			if len(apiResp.Data) > 0 {
				if err := json.Unmarshal(apiResp.Data, out); err != nil {
					return fmt.Errorf("cpm: unmarshal data: %w", err)
				}
			}
		}
		return nil
...
func (c *Client) CreateProxy(ctx context.Context, req CreateProxyRequest) (*ProxySummary, error) {
	...
	if err := c.do(ctx, http.MethodPost, "/api/v1/ipv4-res", req, &out); err != nil {
		return nil, fmt.Errorf("cpm.CreateProxy: %w", err)
	}
	return &out, nil
}
...
// WaitForRunning polls GET /api/v1/ipv4-res/:id until status != "creating".
func (c *Client) WaitForRunning(ctx context.Context, proxyID string) (*ProxySummary, error) {
	deadline := time.Now().Add(pollTimeout)

	for {
		if time.Now().After(deadline) {
			return nil, fmt.Errorf("cpm.WaitForRunning: timeout after %v waiting for proxy %s", pollTimeout, proxyID)
		}
		...
		if err != nil {
			return nil, fmt.Errorf("cpm.WaitForRunning: %w", err)
		}
...
// GET /api/v1/ipv4-res/:id
func (c *Client) GetProxy(ctx context.Context, proxyID string) (*ProxySummary, error) {
	var out ProxySummary
	if err := c.do(ctx, http.MethodGet, "/api/v1/ipv4-res/"+proxyID, nil, &out); err != nil {
		return nil, fmt.Errorf("cpm.GetProxy: %w", err)
	}
	return &out, nil
}
...
func (c *Client) ListProxies(ctx context.Context) ([]ProxySummary, error) {
	var out []ProxySummary
	if err := c.do(ctx, http.MethodGet, "/api/v1/ipv4-res", nil, &out); err != nil {
		return nil, fmt.Errorf("cpm.ListProxies: %w", err)
	}
	return out, nil
}
...
func (c *Client) DeleteProxy(ctx context.Context, proxyID string) error {
	if err := c.do(ctx, http.MethodDelete, "/api/v1/ipv4-res/"+proxyID, nil, nil); err != nil {
		return fmt.Errorf("cpm.DeleteProxy: %w", err)
	}
	return nil
}
...
func (c *Client) StopProxy(ctx context.Context, proxyID string) error {
	if err := c.do(ctx, http.MethodPost, "/api/v1/ipv4-res/"+proxyID+"/stop", nil, nil); err != nil {
		return fmt.Errorf("cpm.StopProxy: %w", err)
	}
	return nil
}
...
func (c *Client) StartProxy(ctx context.Context, proxyID string) error {
	if err := c.do(ctx, http.MethodPost, "/api/v1/ipv4-res/"+proxyID+"/start", nil, nil); err != nil {
		return fmt.Errorf("cpm.StartProxy: %w", err)
	}
	return nil
}

// CheckProxy verifies the proxy exit IP and returns geo info.
// GET /api/v1/ipv4-res/:id/check
func (c *Client) CheckProxy(ctx context.Context, proxyID string) (*CheckResult, error) {
	var out CheckResult
	if err := c.do(ctx, http.MethodGet, "/api/v1/ipv4-res/"+proxyID+"/check", nil, &out); err != nil {
		return nil, fmt.Errorf("cpm.CheckProxy: %w", err)
	}
	return &out, nil
}
...
func (c *Client) ListGroups(ctx context.Context) ([]ProxyGroup, error) {
	...
	if err := c.do(ctx, http.MethodGet, "/api/v1/groups", nil, &out); err != nil {
		return nil, fmt.Errorf("cpm.ListGroups: %w", err)
	}
	return out, nil
}
